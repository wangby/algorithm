...
int main(int argc, char **argv) {
	...
	if (flag_cross_validation) {
		//do_cross_validation();
		double cv = binary_class_cross_validation_ctr(&prob, &samp, &param, nr_fold);
...
void parse_command_line(int argc, char **argv, char *input_file_name,
	...
	// default values
	param.solver_type = L2R_LR; //L2R_L2LOSS_SVC_DUAL;
	param.C = 1;
...
void read_problem(const char *filename) {
	...
	samp.s = Malloc(struct sample_node, samp.l);
	...
	for (i = 0; i < lines; i++) {
		...
		instances++;
		for (int k = 1; k < shows; k++) { // 之前已经插入了一个0
			if (k < non_clicks) {
				prob.y[instances] = 0;
			} else {
				prob.y[instances] = 1;
			...
			memcpy(&x_space[end], &x_space[start],
					feature_len * feature_node_size);
			/*
		...
		samp.s[i].end_index = samp.s[i].start_index + shows - 1;
		samp.s[i].clicks = clicks;
		samp.s[i].shows = shows;
		if (i < lines -1) {
			samp.s[i+1].start_index = samp.s[i].end_index + 1;
		}
	}
	/*
	 for(i=0;i<prob.l;i++) {
...
//	printf("samp.l:%d\n",samp.l);
//	for(i=0;i<samp.l;i++) {
//		 printf("samp.s[%d]:%d %d %d %d \n",i,samp.s[i].clicks, samp.s[i].shows,samp.s[i].start_index,samp.s[i].end_index);
//		 }
