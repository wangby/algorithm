...
double auc_ctr(const dvec_t& dec_values, const ivec_t& clicks,const ivec_t& shows);
...
double auc_ctr(const dvec_t& predicted_ctr, const ivec_t& num_clicks, const ivec_t& num_impressions) {
    double auc  = 0;
	size_t size = predicted_ctr.size();
	size_t i;
	std::vector<size_t> i_sorted(size);

	for(i = 0; i < size; ++i) i_sorted[i] = i;
