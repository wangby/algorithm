...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			#self.svmtrain_pathname = os.path.join(dirname, '../train')
			self.svmtrain_pathname = os.path.join('/usr/local/bin/train_ctr')
			self.gnuplot_pathname = '/usr/bin/gnuplot'
		else:
			# example for windows
			self.svmtrain_pathname = os.path.join(dirname, r'..\windows\train_ctr.exe')
	...
	def parse_options(self, options):
		...
		if not os.path.exists(self.svmtrain_pathname):
			raise IOError('train executable not found')
		...
		if not self.grid_with_c:
			raise ValueError('-log2c should not be null simultaneously')
...
liblinear_options : additional options for train_ctr""")
		sys.exit(1)
