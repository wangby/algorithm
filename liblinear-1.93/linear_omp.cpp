...
void l2r_lr_fun::Xv(double *v, double *Xv)
{
	...
	const int MIN_ITERATOR_NUM = 1000;
	int max_tn = l / MIN_ITERATOR_NUM;
	int tn = max_tn > coreNum ? coreNum : max_tn; //tn表示要设置的线程数量

#pragma omp parallel for if(tn>1) num_threads(tn) private (i)
	for(i=0;i<l;i++)
	{
		feature_node *s=x[i];
		Xv[i]=0;
...
void l2r_l2_svc_fun::subXv(double *v, double *Xv)
{
	...
	int coreNum = omp_get_num_procs();
	const int MIN_ITERATOR_NUM = 1000;
	int max_tn = sizeI / MIN_ITERATOR_NUM;
	int tn = max_tn > coreNum ? coreNum : max_tn; //tn表示要设置的线程数量
#pragma omp parallel for if(tn>1) num_threads(tn) private (i)
	for(i=0;i<sizeI;i++)
	{
...
*/
    int coreNum = omp_get_num_procs();
	const int MIN_ITERATOR_NUM = 10;
	int max_tn = nr_class / MIN_ITERATOR_NUM;
	int tn = max_tn > coreNum ? coreNum : max_tn; //tn表示要设置的线程数量
#pragma omp parallel for if(tn>1) num_threads(tn) private (i)
                for(i=0;i<nr_class;i++)
                {
